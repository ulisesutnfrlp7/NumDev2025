// js/barrioC.js
(async function(){
    const canvas = document.getElementById('chartBarrioC');
    ...
    const colors = {
      ...
      barrio: { bg:'rgba(75,192,192,0.85)', border:'rgba(75,192,192,1)' }
    };
  
    let chart = null;
    async function render(){
      const clusters = await DataLoader.loadXLSX('datos.xlsx');
      const data = clusters.C || [];
      ...
      datasets.push({ label:'Barrio C', data: data, backgroundColor: colors.barrio.bg, borderColor: colors.barrio.border, pointStyle:'cross', radius:7, borderWidth:2 });
  
      const options = {
        responsive:true,
        plugins:{
          title:{display:true,text:'Barrio C — Precio vs m²'},
          legend:{position:'top'},
      ...
      const calcDiv = document.getElementById('calculosC');
      calcDiv.textContent = '';
